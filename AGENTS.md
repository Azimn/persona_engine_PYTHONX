...
- Hardening continuation from preserved `57469a7`: feature branch `wayfarer-local-model-hardening` is backed up remotely. Read `persona_engine/evidence/mvi/MODEL_HARDENING_V2.md` and its frozen failures before repair. Exact Gemma recall denial reproduced twice with selected memory present; diagnose expression/validation separately from retrieval. Human testing remains deferred.
- Current hardening implementation uses v3 model messages with explicit memory speaker/provenance, a distinct current turn, and no authored answer sentences. The v2 builder and original evidence remain available. Recall duties are derived from selected evidence and enforced by one retry/fallback; delivery status distinguishes model output from validation fallback. Tentative inference remains allowed; guards are narrow deterministic fidelity checks, not a semantic judge or a new authority.
- The Ensemble architecture through `b30f490` is integrated into the hardening branch. Its own CI run `33779833026` passed on Python 3.11 and 3.12. Actual-model Ensemble evidence is valid only when candidate authority is `engine_live`, semantic projection matches its offline arm, and no model or engine fallback occurs. Matched probe arms must fork one closed pre-probe subject snapshot so repair wall-clock timestamps are not mistaken for renderer-caused semantic drift.
- Integrated Qwen relationship evidence has 24/24 semantic matches with no fallback and less surface duplication, but all independent samples selected the direct candidate; do not attribute that result to multi-candidate ranking. Gemma categorical denials of subject judgment are frozen in `subject_agency_failures_v3.json` and covered by a generic fidelity guard. Temporary uncertainty remains permitted.
- Final Gemma Ensemble relationship evidence also has 24/24 live-authority semantic matches and no fallback; four candidates were rejected, but mechanistic phrasing remains 3/24 under the current rubric. Qwen and Gemma Scene Lab v2 runs completed without fallback. Read `ENSEMBLE_ACTUAL_MODEL_RESULTS.md` and `ensemble_model_results.json`; these are consumed builder-designed cases, not human or independent efficacy evidence.
- PR #10 promoted the integrated phase to production merge `c7a1b9180c14e72ce6070b439d281d8a8c4c3f30`; normal Wayfarer CI run `33790796255` passed on Python 3.11 and 3.12. Preserved checkpoint `57469a7` remains in production ancestry.
