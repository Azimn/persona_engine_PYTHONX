...
2026-09-03 update: frozen local actual-model collection at `9408351aac938441523534974fc299a75c961604` is complete. Owner-authorized automated development now follows that preserved baseline. The model-hardening and Ensemble branches are integrated; matched Qwen/Gemma relationship runs preserve live semantic authority and reduce narrow repetition measurements, while Gemma still exhibits mechanistic speech. See `WAYFARER_PROGRESS.md`, `evidence/mvi/LOCAL_MODEL_EXPRESSION.md`, and `evidence/mvi/ENSEMBLE_ACTUAL_MODEL_RESULTS.md`. M18 remains in progress. Human testing is deliberately deferred until stronger automated evidence.
...
- [ ] Run small local model. Real Ollama execution/capture plus low-token preflight/smoke operator path now exists; no actual model result recorded yet.
- [ ] Run medium local model where available. Preflight may recommend an already-installed medium candidate, but never pulls or auto-selects a large model; reuse the same frozen requests before changing the fixture.
