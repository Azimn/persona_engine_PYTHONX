...
class DuckOrganism:
    def __init__(self, subject: SubjectPort, *, organism_id: str | None = None, config: DuckConfig | None = None,
                 drives: DriveSystem | None = None, world_model: RuleWorldModel | None = None,
                 services: ServiceRegistry | None = None, procedures: ProcedureRegistry | None = None,
                 execution_policy: ExecutionPolicy | None = None, embodiment: EmbodimentCapabilities | None = None,
                 persistence: DuckPersistence | None = None, state: OrganismState | None = None):
        ...
        self.scheduler = CognitiveScheduler(drive_threshold=self.config.endogenous_drive_threshold,
        ...
        if state is None:
            state = OrganismState(schema_version=self.config.schema_version, organism_id=organism_id or str(uuid4()),
                                  subject_id=subject.subject_id, drive_state=self.drives.drives,
    ...
    def current_state(self) -> OrganismState: return self.state
    def current_broadcast(self): return self.workspace.last_broadcast
    def set_services(self, services: ServiceRegistry) -> None: self.services = services
    def ingest(self, event: ExternalEvent) -> None: self.scheduler.ingest(event)
    ...
    def _patch(self, *, domain: str, old_value, new_value, source: str, reason: str, evidence: tuple[str, ...] = ()) -> StatePatch:
        return StatePatch(domain=domain, key=domain, old_value=old_value, new_value=new_value,
                          source_module=source, reason=reason, evidence_refs=evidence,
                          tick=self.state.tick, authorization_class="duck_internal")

    def _memory_query(self, trigger: ExternalEvent) -> str:
        payload = trigger.payload
        return str(payload.get("observed_text") or payload.get("description") or payload.get("topic") or payload.get("unresolved_question") or "").strip()
    ...
    def step(self, *, budget_ms: int | None = None) -> CycleTrace | None:
        ...
        if self.config.enable_memory_activation:
            items.extend(self.memory_activation.retrieve(self.subject, query=self._memory_query(trigger), now=trigger.timestamp,
                                                         tick=tick, subject_id=self.state.subject_id,
                                                         top_k=self.config.memory_retrieval_width))
        service_projection = {
            "trigger": trigger.to_dict(), "attribution": attribution_frame.to_dict(),
            "situation": self.state.situation.to_dict(), "broadcast_history": self.state.working_memory[-3:],
        ...
        service_items, service_errors = self.services.proposals(ServiceContext(tick=tick, subject_id=self.state.subject_id,
        ...
        if broadcast is not None:
            ...
            patch = self._patch(domain="working_memory", old_value=list(self.state.working_memory), new_value=next_working,
                                source="workspace", reason="global broadcast changes bounded working memory",
                                evidence=(broadcast.winner.item_id,))
            self.reducer.apply(self.state, patch); patches.append(patch)
        ...
        deduped = {action.action_id: action for action in actions}; actions = [deduped[key] for key in sorted(deduped)]
        context = {"tick": tick, "situation": self.state.situation.to_dict(), "subject": self.subject.snapshot(),
        ...
        if self.config.enable_simulation:
            ...
            simulations = [SimulationResult(action_id=action.action_id, predicted_world_effects=dict(action.expected_world_effects),
                                            predicted_self_effects=dict(action.expected_self_effects), confidence=0.25,
                                            provenance={"source": "simulation_lesion", "action_type": action.action_type}) for action in actions]
        ...
        intention_patch = self._patch(domain="current_intention", old_value=self.state.current_intention, new_value=intention,
                                      source="action_selector", reason="serialized action commitment after simulation",
                                      evidence=(broadcast.winner.item_id,) if broadcast else ())
        self.reducer.apply(self.state, intention_patch); patches.append(intention_patch)

        execution = self.executor.execute(action, simulation, context)
        observed_world, observed_self = execution.world_effects, execution.self_effects
        ...
        prediction = PredictionRecord(prediction_id=f"prediction:{tick}:{action.action_id}", intention_id=intention.intention_id,
                                      predicted_world_effects=dict(simulation.predicted_world_effects), predicted_self_effects=dict(simulation.predicted_self_effects),
                                      observed_world_effects=dict(observed_world), observed_self_effects=dict(observed_self),
                                      world_error=world_error, self_error=self_error)
        ...
        self.metacognition.observe(world_error=world_error, self_error=self_error, simulation_confidence=simulation.confidence)

        action_entry = {"tick": tick, "intention": intention.to_dict(), "execution": execution.to_dict(),
                        "outcome": {"world": observed_world, "self": observed_self, "drive_effects": applied_drives}}
        for patch in [
            self._patch(domain="action_ledger", old_value=list(self.state.action_ledger),
                        new_value=(list(self.state.action_ledger) + [action_entry])[-128:], source="executor",
                        reason="record observed action outcome", evidence=(trigger.event_id,)),
            self._patch(domain="prediction_ledger", old_value=list(self.state.prediction_ledger),
                        new_value=(list(self.state.prediction_ledger) + [prediction.to_dict()])[-128:], source="prediction",
                        reason="compare expected and observed world/self effects", evidence=(intention.intention_id,)),
        ]:
            self.reducer.apply(self.state, patch); patches.append(patch)
        self.subject.advance_time(1.0)
        for patch in [
            self._patch(domain="tick", old_value=self.state.tick, new_value=self.state.tick + 1, source="scheduler",
                        reason="complete one serialized cognitive cycle", evidence=(trigger.event_id,)),
            self._patch(domain="scheduler_state", old_value=dict(self.state.scheduler_state), new_value=self.scheduler.snapshot(),
                        source="scheduler", reason="persist bounded endogenous scheduling state"),
        ]:
            self.reducer.apply(self.state, patch); patches.append(patch)

        trace = CycleTrace(
            tick=tick, trigger=trigger.to_dict(),
            situation_changes={**situation_changes, "attribution": attribution_frame.to_dict(), "subject_observation": subject_result},
            drive_changes=drive_changes, cognitive_items=tuple(item.to_dict() for item in items),
            broadcast=broadcast.to_dict() if broadcast else None, action_candidates=tuple(item.to_dict() for item in actions),
            simulations=tuple(item.to_dict() for item in simulations), selected_intention=intention.to_dict(),
            outcome={"execution": execution.to_dict(), "world": observed_world, "self": observed_self, "drive_effects": applied_drives},
            prediction=prediction.to_dict(), patches=tuple(item.to_dict() for item in patches),
            service_errors=tuple(service_errors), service_proposals=tuple(item.to_dict() for item in service_items),
        )
        self.traces.append(trace)
        if self.persistence is not None:
            self.persistence.append_trace(trace); self.persistence.save(self.state)
        return trace
    ...
    def metacognitive_report(self) -> dict:
        ...
        return {
            "subject_id": self.state.subject_id, "organism_id": self.state.organism_id, "tick": self.state.tick,
            ...
            "latest_prediction": latest, "calibration": self.metacognition.report(),
            "world_model_reliability": dict(sorted(self.world_model.reliability.items())),
            "service_count": len(self.services.services),
            "lesions": {"motivation": not self.config.enable_motivation, "memory_activation": not self.config.enable_memory_activation,
                        "workspace": not self.config.enable_workspace, "simulation": not self.config.enable_simulation},
        }
